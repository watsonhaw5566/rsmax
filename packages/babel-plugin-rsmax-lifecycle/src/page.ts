...
const defaultLifecycleEvents = ['onPageScroll', 'onShareAppMessage', 'onShareTimeline'];
...
export default (options: Options) => {
  ...
  function record(importer: string, name: string) {
    ...
    Store.pageEvents.set(importer, Store.pageEvents.get(importer)?.add(name) ?? new Set([name]));
  }
  return declare(() => {
    return {
      name: 'babel-plugin-rsmax-lifecycle-page',
      pre(state: any) {
      ...
      visitor: {
        ...
        CallExpression(path: NodePath<t.CallExpression>, state: any) {
          ...
          if (callee.type === 'Identifier' && (callee.name === 'usePageEvent' || callee.name === 'useAppEvent')) {
        ...
        StringLiteral(path: NodePath<t.StringLiteral>, state: any) {
          ...
          if (
            ...
            ((parent.callee as t.Identifier).name === 'usePageEvent' ||
