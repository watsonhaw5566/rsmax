...
Rsmax 应用的默认入口文件为 `src/app.js`。不同于原生小程序中的 `app.js`，Rsmax 中的 `app.js` 是一个 React 组件。
...
> 注意
>
> 在 Rsmax 中使用 `getApp` 是获取不到 `src/app.js` 中定义的 `App` 组件的。 我们建议你忘记 `getApp`， 改用 `Context` 来共享状态。
>
...
Rsmax 优先读取默认导出的配置，如果你开发的是一个跨平台的的项目，则可以改为：
