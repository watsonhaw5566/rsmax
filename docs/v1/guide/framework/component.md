...
如果小程序添加了新的组件，而你所用的 Rsmax 版本还没提供该组件的支持，Rsmax 允许你自己创建一个新的基础组件。

假设小程序新增了一个 `<foo-bar>` 组件，你可以通过 `createHostComponent` 这个方法把这个组件注册到 Rsmax 中：
