...
[小程序生命周期未调用？](/v1/faq#使用高阶组件导致页面的生命周期未调用)
...
```jsx
import { usePageInstance } from 'Rsmax'
...
> 注意
>
> Rsmax 在页面实例上设置了一些内部逻辑相关的属性（包括 data 上面的值），不要修改实例上的属性。

## 小程序渲染 Effect

一般在 React 里，我们通过在 `useEffect` 来进行页面渲染成后需要处理的逻辑。但是在 Rsmax 中，`useEffect` 只是代表了 Rsmax 虚拟 DOM 处理完成，并不代表小程序渲染完成。
为了支持开发者可以触及小程序渲染完成的时机，我们添加了一个新的 hook：`useNativeEffect`。

```jsx
import * as React from 'react';
import { useNativeEffect } from 'Rsmax';
import { usePageEvent } from 'Rsmax/macro';
