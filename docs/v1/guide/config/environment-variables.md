...
rsmax 会在编译时注入 `NODE_ENV` 以及所有 `RSMAX_APP_` 开头的环境变量，你可以通过 `process.env` 来读取这些环境变量。

> 注意，只有 `RSMAX_APP_` 开头的环境变量才会被注入。

使用环境变量启动构建：

```
$ RSMAX_APP_BASE_URL=https://example.com/api rsmax build -t ali
```

**Windows** 用户建议使用 [cross-env](https://www.npmjs.com/package/cross-env) 来设置环境变量：

```
$ cross-env RSMAX_APP_BASE_URL=https://example.com/api rsmax build -t ali
```
...
function request(options) {
  my.request({
    url: process.env.RSMAX_APP_BASE_URL + options.path,
    success(res) {},
    fail(e) {},
  });
}
...
为了方便管理环境变量，`@rsmax/cli` 内置了 [dotenv](https://github.com/motdotla/dotenv)，你可以通过在项目根目录下创建 `.env` 文件方便地定义环境变量:

```
RSMAX_APP_BASE_URL=https://example.com/api
```
