...
为了与小程序对齐，Rsmax 为 Web 平台也提供了一份与小程序类似的应用配置。
...
同样，Rsmax 为 Web 平台也提供了一份与小程序类似的页面配置。
...
> 注意
>
> web 平台只能通过 rsmax/one 来开发，因此 rsmax 给 web 平台提供的组件和 api 就是来自 rsmax/one 提供的组件和 api

## 样式

### 样式隔离

与小程序不同，web 中页面之间的样式是没有隔离的，因此建议用 [CSS modules](/v1/guide/framework/style#css-modules) 来解决这个问题。
