...
`rsmax/macro` 下包含的是基于 [babel-plugin-macros](https://github.com/kentcdodds/babel-plugin-macros) 的宏。
...
// 假设微信提供了一个 rsmax 还没有支持的新组件 custom-view，则可以通过 createHostComponent 创建：
...
- 数组对象可以是一个数组，用来给原生属性做别名，形如：`['class', 'className']`，表示在 rsmax 中用 `className` 代替 `class`
...
```js
import { requirePlugin } from 'rsmax/macro';
