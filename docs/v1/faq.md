...
rsmax 支持直接使用原生组件库。具体请参考 [小程序自定义组件](/v1/guide/basic/custom-component)。

## rsmax 支持跨平台开发吗？

支持。

具体可参考 [跨平台开发](/v1/guide/one/)。
...
对于使用 class 组件的页面，rsmax 会通过 ref 获取页面生命周期。当页面组件被高阶组件（有时被称为 HOC）包裹时，通过 ref 取得的是 HOC 最外层的容器组件，而不是被包裹的组件 。我们需要 `React.forwardRef` 将 ref 转发到内部组件。
...
rsmax 默认为微信的每个 host 组件定义了嵌套层数。其中 `View` 20 层，其他组件都在 1 ~ 5 层。如果出现形如下图的情况，可以通过配置修改嵌套层数。
...
## 小程序开发模式下 `rsmax build --watch` 无法真机预览?
...
Taro 3.0 版本的实现原理与 rsmax 大同小异，没有太大的区别。主要区别在于以下两个方面：

- **跨平台设计**，我们深知要打造一个面面俱到的多平台框架会有非常多的坑，与其挖一个大坑填不上，我们选择把有限的精力放在框架的核心功能上，并提供跨平台同构机制以及一个精简的跨平台实现（rsmax One），让开发者可以根据自己的业务需求去做跨平台实现。
- **专注 React**，因为 React 是我们自己大量使用和熟悉的前端框架，rsmax 会持续专注在 React 技术栈。
